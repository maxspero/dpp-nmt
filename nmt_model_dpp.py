...
class NMT(nn.Module):
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            ###### START HERE #######
            top_cand_hyp_scores, top_cand_hyp_pos = torch.topk(contiuating_hyp_scores, k=5*live_hyp_num)
            ...
            new_cell_t = new_h_t[top_cand_hyp_pos]
            ...
            norms = torch.norm(h_t_dpp, p=2, dim=1, keepdim=True).detach()
            ...
            features = unit_vectors * torch.exp(top_cand_hyp_scores.unsqueeze(1)).expand_as(unit_vectors)
            L = torch.mm(features, features.t())
            print("L", L.shape)
            new_top_cand_hyp_pos = sample_k_dpp(L, k=live_hyp_num)
            print(top_cand_hyp_pos)
            new_top_cand_hyp_scores = contiuating_hyp_scores[new_top_cand_hyp_pos]
            print("new_top_hyp_pos", new_top_cand_hyp_pos.shape)
            print("new_top_hyp_scores", new_top_cand_hyp_scores.shape)
