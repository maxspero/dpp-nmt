...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X_new = nn.utils.rnn.pack_padded_sequence(X, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X_new)
        enc_hiddens, batch_lens = nn.utils.rnn.pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)
        last_hidden_cat = torch.cat((last_hidden[0,:,:], last_hidden[1,:,:]), 1)
        init_decoder_hidden = self.h_projection(last_hidden_cat)
        last_cell_cat = torch.cat((last_cell[0,:,:], last_cell[1,:,:]), 1)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            # print(hypotheses)
            ###### START HERE #######
            top_cand_hyp_scores, top_cand_hyp_pos = torch.topk(contiuating_hyp_scores, k=3*live_hyp_num)


            # for every element in contiuating_hyp_scores, I need to get the targe
